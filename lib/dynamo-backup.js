...
function DynamoBackup(options) {
    ...
    this.streamToStdout = options.streamToStdout || false;
        ...
        function (items) {
            items.forEach(function (item) {
                ...
                if (self.saveDataPipelineFormat) {
                    formattedItem = self._formatForDataPipeline(item);
                } else {
                    formattedItem = JSON.stringify(item);
                }

                if(self.streamToStdout){
                  process.stdout.write(formattedItem+'\n');
                } else {
                  stream.append(formattedItem);
                  stream.append('\n');
                }
            });
        },
