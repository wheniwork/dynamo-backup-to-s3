...
DynamoRestore.prototype.resumeReading = function() {    
      //console.log('Resuming back:');
...
DynamoRestore.prototype._processLine = function(line) {
    ...
    if (this.requests.length >= this.options.concurrency) {
        ...
        this.readline.pause();
	    setTimeout(this.resumeReading.bind(this), 800);
    }
...
DynamoRestore.prototype._sendBatch = function() {
    ...
    this.requests.push(dynamo.batchWriteItem(params, (function(error, data) {
        ...
        if (this.tableready && this.requests.length < options.concurrency * 0.3) {
	    //console.log("resuming back request length: "+ this.requests.length);
            //this.readline.resume();
        }
